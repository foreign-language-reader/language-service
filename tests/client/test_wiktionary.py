...
def test_can_parse_definitions(mocker):
    ...
    compare(
        ...
        [
            Definition(
                token="something", subdefinitions=subdefinitions, examples=examples, tag=part_of_speech
            ),
            Definition(token="something", subdefinitions=None, examples=None, tag="NOUN"),
        ],
    )
